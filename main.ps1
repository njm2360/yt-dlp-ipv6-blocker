# Google IPv6 Blocker for yt-dlp.exe (VRChat)
...
# Firewall rule and target program path
$ruleName    = 'Block-Google-IPv6-for-yt-dlp'
$programPath = Join-Path $env:USERPROFILE 'AppData\LocalLow\VRChat\VRChat\Tools\yt-dlp.exe'
...
if (-not $rule) {
    ...
    # Toggle state
    $current = (Get-NetFirewallRule -DisplayName $ruleName).Enabled -eq 'True'
    if ($current) {
